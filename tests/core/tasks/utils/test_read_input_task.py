...
class PrepareRicoScaTest(IsolatedAsyncioTestCase):
    ...
    async def test_multi_task_wrong_probing_model(self):
    ...
    async def test_multi_task_wrong_inter_metric(self):
    ...
    async def test_multi_task_wrong_intra_metric(self):
    ...
    async def test_wrong_control_size(self):
    ...
    async def test_missing_key(self):
        ...
        with self.assertRaises(SystemExit):
            await read_input_task.run(TEST_INPUT_1)

    async def test_wrong_key_name(self):

        TEST_INPUT_2 = "./tests/sample_files/test_input/wrong_template_format.json"
        read_input_task = ReadInputTask()

        with self.assertRaises(SystemExit):
            await read_input_task.run(TEST_INPUT_2)
    ...
    async def test_wrong_json_format(self):
